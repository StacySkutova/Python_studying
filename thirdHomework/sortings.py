...
def InsertSort(A): # алгоритм исправлен, поскольку не всегда отсортировывал все, особенно пограничные элементы - пример, 5000, 2999 и т.д
...
# реализованный на паре алгоритм (подправлен)
...
def ShakerSort(A):  # подсмотрено и изучено отсюда - https://habr.com/ru/post/414653/ - теперь логично + времени занимает примерно как и пузырек
    # while True:
    ...
    for i in range(len(A)//2):  # исправленный вариант
        for j in range(len(A) - 1 - i):  # тут сперва неверно поняла - не нужно от i in range(i, len(A) - 1 - i)
            if A[j] > A[j + 1]:
                a = A[j]
                A[j] = A[j + 1]
                A[j + 1] = a
        for j in range(len(A) - 2 - i, i + 1, -1):  # тут сперва неверно поняла - нужна еще -1 в конце in range(len(A) - 2 - i, i + 1)
            if A[j] < A[j - 1]:
                a = A[j]
                A[j] = A[j - 1]
                A[j - 1] = a
